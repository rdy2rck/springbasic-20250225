...
public class WebSecurityConfig {
  ...
  throws Exception {
    ...
    security
      ...
      // Session:
      // 웹 어플리케이션에서 클라이언트에 대한 정보를 유지하기 위한 기술
      ...
      // - 클라이언트가 자신의 의도와는 무관하게 공격 행위를 하는 것
      ...
      // 인가 작업
      // - 요청 URL의 패턴(자원)에 따라 인가 작업이 필요한 작업인 지 지정하는 설정
      // - 모든 클라이언트가 접근할 수 있도록 허용
      // - 인증된 모든 클라이언트가 접근할 수 있도록 허용
      // - 인증된 클라이언트 중 특정 권한을 가진 클라이언트만 접근할 수 있도록 허용
      .authorizeHttpRequests(request -> request
        // requestMatchers(): URL 패턴, HTTP 메서드, URL 패턴 + HTTP 메서드 마다 접근 권한을 부여하는 메서드
        // permitAll(): 모든 클라이언트가 접근할 수 있도록 지정
        // authenticated(): 인증된 모든 클라이언트가 접근할 수 있도록 지정
        // hasRole(권한): 특정 권한을 가진 클라이언트만 접근할 수 있도록 지정 (매개변수로 전달하는 권한 명은 'ROLE_' 를 제거한 실제 권한명)
