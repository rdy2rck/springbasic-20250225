...
// - 암호화가 되어 있어 클라이언트와 서버 간에 안전한 데이터 전송을 수행
// - 헤더 : 토큰 유형, 서명에 사용된 암호화 알고리즘이 지정되어 있음
// - 페이로드 : 클라이언트 혹은 서버가 전달할 데이터가 포함되어 있음
// - 서명 : 헤더와 페이로드를 인코등하여 합치고 비밀 키로 암호화한 데이터
@Component
public class JwtProvider {

  // JWT 암호화에 사용되는 비밀 키는 보안 관리되어야 함
  // 코드 상에 직접 작성하는 것은 보안상 좋지 않음
  
  // 해결책
  // 1. application.properties / application.yaml에 등록
  // - application.properties / application.yaml에 비밀 키를 작성
  ...
  // - OS 자체의 시스템 환경 변수에 비밀 키를 등록
  // - Spring에서 시스템 환경 변수를 읽어서 사용

  // 3. 외부 데이터 관리 도구를 사용
  // - 자체 서버가 아닌 타 서버에 등록된 Vault 도구를 사용하여 비밀 키 관리
  // - OS 부팅 시에 Vault 서버에서 비밀 키를 가져와서 사용함
  // - OS 부팅 시 마다 새로운 비밀 키를 자동으로 부여함
