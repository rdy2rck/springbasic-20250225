...
public class SecurityServiceImplement implements SecurityService {
  ...
  // - 비밀 번호를 쉽고 안전하게 암호화하여 관리할 수 있도록 도움을 주는 인터페이스
  // - 구현체: BCryptPasswordEncoder(가장 흔함), ScryptPasswordEncoder, Pbkdf2PasswordEncoder
  // - String encode(평문비밀번호): 평문 비밀번호를 암호화하여 반환
  // - boolean matches(평문비밀번호, 암호화된비밀번호): 평문비밀번호와 암호화된 비밀번호가 서로 일치하는 지 확인
  ...
  public ResponseEntity<String> signUp(SignUpRequestDto dto) {
    ...
    try {
      ...
      if (isExistedUserId) return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("존재하는 아이디");

      String userTelNumber = dto.getUserTelNumber();
      boolean isExistedUserTelNumber = userRepository.existsByUserTelNumber(userTelNumber);
      if (isExistedUserTelNumber) return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("존재하는 전화번호");
