// 기능 : 사용자가 입력한 데이터 중복 검사 & 결과 응답 생성
...
public class UserServiceImplement implements UserService {
  ...
  public void queryMethod() {
    ...
    // deletedById(아이디데이터) : 아이디 기준으로 레코드 삭제
    userRepository.deleteById("아이디");

    // delete(엔터티) : 해당 엔터티 레코드를 삭제
    userRepository.delete(userEntity);

    // deleteAll(엔터티컬렉션) : 해당하는 엔터티 리스트를 삭제
  ...
  public ResponseEntity<ResponseDto> postUser(PostUserRequestDto dto) {
    ...
    try {
        ...
        // 빌더 패턴 : 객체 생성 과정에 멤버변수 별로 객체를 구성한 후 객ㅊ를 생성할 수 있도록 도움을 주는 생성 패턴
